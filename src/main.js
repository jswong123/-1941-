...
function ensureSaveControls() {
    if (document.getElementById("save-system-controls")) return;
    const style = document.createElement("style");
    style.id = "save-system-style";
    style.textContent = `
        #save-system-controls{position:fixed;top:12px;right:12px;z-index:9999;display:flex;gap:6px;padding:7px;border:1px solid #686451;background:rgba(214,207,178,.94);box-shadow:0 3px 12px rgba(0,0,0,.25);font-family:FangSong,STFangsong,SimSun,serif}
        #save-system-controls button,#save-system-controls select{border:1px solid #625f50;background:#eee8d1;color:#282820;padding:5px 9px;font:inherit;cursor:pointer}
        #save-system-controls button:hover{background:#d9d1b5}
        #save-system-controls button:disabled{opacity:.45;cursor:default}
    `;
    document.head.appendChild(style);
    const panel = document.createElement("div");
    panel.id = "save-system-controls";
    panel.innerHTML = `
        <select id="save-slot-select" aria-label="存档槽位">
            <option value="1">存档 1</option><option value="2">存档 2</option><option value="3">存档 3</option>
            <option value="4">存档 4</option><option value="5">存档 5</option><option value="6">存档 6</option>
        </select>
        <button id="save-game-button" type="button">保存</button>
        <button id="load-game-button" type="button">读取</button>
        <button id="undo-game-button" type="button">撤销</button>
    `;
    document.body.appendChild(panel);
    const selectedSlot = () => Number(document.getElementById("save-slot-select")?.value ?? 1);
    document.getElementById("save-game-button")?.addEventListener("click", () => saveGame(selectedSlot()));
    document.getElementById("load-game-button")?.addEventListener("click", () => {
        if (!loadGame(selectedSlot())) alert("该槽位没有可读取的存档。");
    });
    document.getElementById("undo-game-button")?.addEventListener("click", undoLastAction);
    document.getElementById("save-slot-select")?.addEventListener("change", updateSaveControls);
    updateSaveControls();
}
